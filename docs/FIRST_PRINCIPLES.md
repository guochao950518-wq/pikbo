...
| 层 | 状态 |
|----|------|
| 需求（Step 1） | 地基已对齐；本文件冻结蠢需求；**调研站只作对照不类比堆功能** |
| 删除（Step 2） | 导航降级非关键路径；禁止 Character 云 / 假多模型 / 自研 3D |
| 简化（Step 3） | contracts / pricing / Mini free；**Toy Identity = 2 字段进 extra** |
| 加速（Step 4） | 本机 live ✅；出片后 **Same photo · next job**；公网部署仍老板门 |
| 自动化（Step 5） | preflight 有；CI 次要；**不做自动识 SKU / 自动发帖** |

**2026-07-24 产品刀：** 见 `docs/research/FIVE_STEP_TOY_IDENTITY.md`（Identity + 交付清单 + 同图换 job）。
